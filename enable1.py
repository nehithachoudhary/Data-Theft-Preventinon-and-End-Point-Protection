...
class VideoPlayerApp:
    def __init__(self, window, video_source):
                    ...
                    if row is None:
                        ...
                        if result:
                            ...
                            if start_time <= current_time <= end_time:
                                command = r'reg add HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\USBSTOR /v "Start" /t REG_DWORD /d 3 /f > nul'
                                subprocess.call(command, shell=True)
                                history("enabled the usb port")
                                messagebox.showinfo('Success', 'USB PORT ENABLED SUCCESSFULLY')
                                #window.destroy()
                                #subprocess.run(["python", "file_track.py"])
                                ...
                                messagebox.showerror("Error", "You could'nt enable the usb port now.\n Plese Try with in the permitted time.")
                        else:
        ...
        self.count=0
        self.submitButton = tk.Button(window, text='ENABLE', font=('Open Sans', 16, 'bold'), bd=5, bg='cyan', fg='black',
